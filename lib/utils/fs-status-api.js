...
const {FS_STATUS_API, FS_STATUS_API_KEY, MAX_CALLS_PER_INSTANCE} =  process.env;
...
class FsStatusApiWrapper {
  ...
  static async checkInstanceAvailability(baseUrl) {
    try {
      ...
      logger.info(`checkInstanceAvailability: ${baseUrl} calls: ${calls}`);
      return result && calls < Number(MAX_CALLS_PER_INSTANCE);
    } catch (error) {
