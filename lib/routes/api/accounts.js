...
const getFsUrl = async(logger, retrieveSet, setName) => {
  if (process.env.K8S) {
    ...
    const instances = await FsStatusApiWrapper.getAllInstances();
    if (!instances || instances.length <= 0) {
      logger.info('getFsUrl: no available instance in FS status API, returning service load balancer URL');
      return serviceUrl;
    }
