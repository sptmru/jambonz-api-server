...
const getFsUrl = async(logger, retrieveSet, setName) => {
  if (process.env.K8S) {
    ...
    const serviceUrl = `http://${process.env.K8S_FEATURE_SERVER_SERVICE_NAME}:${port}/v1/createCall`;
    const availableInstances = await FsStatusApiWrapper.getAvailableInstances();
    if (!availableInstances || availableInstances.length <= 0) {
      logger.info('getFsUrl: no available instances');
      return false;
    }

    const shuffledInstances = shuffle(availableInstances);

    for (const instanceData of shuffledInstances) {
      const instanceIPAddress = instanceData.instanceId;
      const instanceBaseUrl = `http://${instanceIPAddress}:${port}`;

      if (await FsStatusApiWrapper.checkInstanceAvailability(instanceBaseUrl)) {
        logger.info(`getFsUrl: instance ${instanceIPAddress} is available, returning instance URL`);
        return `${instanceBaseUrl}/v1/createCall`;
      } else {
        logger.info(`getFsUrl: instance ${instanceIPAddress} is not available, deleting instance data`);
        await FsStatusApiWrapper.deleteInstanceData(instanceIPAddress);
      }
    }
...
const createCall = async(req, res, serviceUrl, logger, retrieveSet, setName) => {
  try {
    ...
    request({
      ...
      const instanceIpAddress = extractIpAddress(serviceUrl);

      if (err || response.statusCode !== 201) {
        logger.error(`createCall: error when creating a new call: ${err}`);
        const newServiceUrl = await getFsUrl(logger, retrieveSet, setName);
        logger.info(`createCall: retrying with new FS instance ${newServiceUrl}`);
        void FsStatusApiWrapper.deleteInstanceData(instanceIpAddress);
        logger.info(`createCall: remove failed instance ${instanceIpAddress}`);
        if (!newServiceUrl) {
          res.set('Retry-After', 60);
          return res.status(429).json({error: 'No available feature servers to handle createCall API request'});
        }
        return createCall(req, res, newServiceUrl, logger, retrieveSet, setName);
      }

      logger.info(`createCall: call created successfully on ${instanceIpAddress}`);
...
router.post('/:sid/Calls', async(req, res) => {
  ...
  return createCall(req, res, serviceUrl, logger, retrieveSet, setName);
});
